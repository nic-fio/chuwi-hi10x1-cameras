 ...
 *
 * Struttura basata su drivers/media/i2c/gc05a2.c (Zhi Mao, MediaTek),
 * stesso produttore di sensori. Adattamento x86/ACPI basato su
 * drivers/media/i2c/ov2740.c, che gira sulla stessa catena IPU6.
 *
 * Registri derivati dalla patch out-of-tree per Alder Lake-M in
 * intel/ipu6-drivers, che a sua volta deriva dalla serie ChromeOS di
 * Tomasz Figa. L'attribuzione va concordata prima di qualsiasi invio:
 * vedi reference/README.md nel progetto.
 */
...
/*
 * Il GC5035 usa indirizzi di registro a 8 bit con banchi di pagina:
 * il registro 0xfe seleziona la pagina. Tutti i registri sotto sono in
 * pagina 0 salvo dove indicato.
 */
...
/*
 * TODO BLOCCANTE: confermare dalla DSDT del CHUWI Hi10 X1.
 * 438 MHz e 2 lane vengono dalla patch Intel per Alder Lake-M, piattaforma
 * diversa. Precedente che dimostra che il valore puo' essere machine-specific:
 * commit fb16c04a538e cambia la link frequency di ov2740 a 180 MHz sulle
 * macchine che usano ipu-bridge, contro i 360 MHz dei Chromebook.
 *
 * V4L2_CID_LINK_FREQ e' il clock DDR, meta' del bit rate per lane:
...
/* TODO: confermare dal _DSD clock-frequency. Su ADL-N e' spesso 19.2 MHz. */
#define GC5035_MCLK_DEFAULT		(24 * HZ_PER_MHZ)
...
/*
 * Su x86 questi regolatori li fornisce INT3472 quando i pin corrispondenti
 * sono descritti nei _DSM. Se non lo sono, il core restituisce dei dummy e
 * il bulk_get riesce comunque.
 */
...
/*
 * TODO BLOCCANTE: importare la sequenza completa dalla patch Intel
 * (reference/gc5035-intel-adlm.patch, ~600 righe di tabelle).
 *
 * Non e' un copia-incolla: i registri di PLL e timing D-PHY sono blob non
 * documentati, calcolati per la link frequency di ADL-M. Se la DSDT del CHUWI
 * dichiara valori diversi non si sa quali byte cambiare — vedi
 * docs/05-parametri-sensori.md, "Rischio aperto".
 *
 * L'attribuzione va concordata prima: reference/README.md.
 */
static const struct cci_reg_sequence gc5035_mode_2592x1944[] = {
	{ GC5035_REG_PAGE_SELECT, GC5035_PAGE_0 },
};
...
/*
 * Solo la full resolution. Le mode binned della patch Intel (1296x972 e
 * 1280x720) dichiarano hts 1460 e 1896 ma scrivono lo stesso line length di
 * 2920 nei registri: l'HBLANK esposto sarebbe sbagliato. Vanno aggiunte solo
 * dopo aver verificato i registri reali sull'hardware.
 */
...
static int gc5035_power_off(struct device *dev)
{
	...
	/*
	 * Ordine invertito rispetto a gc05a2: su INT3472 il clock e' spesso un
	 * clk-gpio che coincide con l'enable del modulo, quindi va spento dopo
	 * aver asserito reset e powerdown.
	 */
 ...
 *   pixel_rate = link_freq * 2 * nr_lanes * 16 / k / bpp,  k = 16 per D-PHY
 * cioe' link_freq * 2 * lanes / bpp.
 *
 * Controprova con il timing del mode, che e' il vincolo che conta davvero
 * perche' V4L2_CID_PIXEL_RATE viene usato insieme a HBLANK/VBLANK per
 * calcolare il frame interval:
 *   hts * vts * fps = 2920 * 2008 * 30 = 175,9 MHz
 *   link_freq * 2 * lanes / bpp = 438e6 * 2 * 2 / 10 = 175,2 MHz
 * I due valori concordano, quindi la formula del bus e' utilizzabile qui.
 */
...
static int gc5035_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN:
		/*
		 * TODO BLOCCANTE: 0xb6 non prende un moltiplicatore, prende un
		 * INDICE in GC5035_AGC_Param[17][2] (soglia -> codice), e il
		 * resto va compensato in digital gain su 0xb1/0xb2. La tabella
		 * va importata dalla patch Intel. Finche' non c'e', il
		 * controllo e' limitato a 1x in init_controls.
		 */
		ret = -EOPNOTSUPP;
		break;
...
static int gc5035_parse_fwnode(struct gc5035 *gc5035)
{
	...
	/*
	 * Su IPU6 questo grafo non viene dall'ACPI: lo sintetizza ipu-bridge,
	 * ma solo per gli HID presenti in ipu_supported_sensors[].
	 *
	 * -EPROBE_DEFER, non -EINVAL: il sensore puo' fare probe prima che
	 * ipu-bridge abbia costruito il grafo. Fallire qui in modo definitivo
	 * rende il driver non caricabile per una pura questione di ordine di
	 * probe. Stesso rimedio usato da ov2680 su x86.
	 */
	...
	/*
	 * Le tabelle registri configurano il blocco MIPI in pagina 3 per un
	 * numero fisso di lane, quindi qui non si puo' accettare qualunque
	 * valore: si verifica che il firmware dichiari quello per cui le
	 * tabelle sono scritte. Il conteggio viene poi conservato e usato per
	 * il calcolo del pixel rate, invece di essere riassunto in una
	 * costante.
	 */
...
static int gc5035_init_controls(struct gc5035 *gc5035)
{
	...
	/*
	 * TODO BLOCCANTE: range limitato a 1x finche' la tabella
	 * GC5035_AGC_Param non e' importata. Vedi gc5035_set_ctrl().
	 */
...
static int gc5035_probe(struct i2c_client *client)
{
	...
	/*
	 * Su x86 li fornisce INT3472 e possono non esserci entrambi.
	 * Verificato sul CHUWI Hi10 X1 leggendo INT3472:02 a runtime:
	 * "reset" c'e' (pin 175, active-low, tipo RESET 0x00), e c'e' un
	 * POWER_ENABLE mappato a regolatore "avdd". Nessun "powerdown"
	 * osservato finora, da cui _optional su entrambi.
	 */
